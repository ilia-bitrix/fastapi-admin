...
from app.other_apps.fastapi_admin.depends import get_current_admin, get_redis, get_resources
from app.other_apps.fastapi_admin.i18n import _
from app.other_apps.fastapi_admin.models import AbstractAdmin
from app.other_apps.fastapi_admin.providers import Provider
from app.other_apps.fastapi_admin.template import templates
from app.other_apps.fastapi_admin.utils import check_password, hash_password
...
class UsernamePasswordProvider(Provider):
    ...
    async def pre_save_admin(self, _, instance: AbstractAdmin, using_db, update_fields):
        """ check is user in db or True check old passwor if False generate news hash
                                   False generate news password
            !TODO:
                - Add generate log.
        """
        try:
            db_obj = await instance.get(email=instance.email)
    ...
    async def login(self, request: Request, redis: Redis = Depends(get_redis)):
        ...
        await redis.set(constants.LOGIN_USER.format(token=token), admin.email, expire=expire)
        return response
    ...
    def redirect_init(self, request: Request):
        return RedirectResponse(
            url=request.app.admin_path + '/init', status_code=HTTP_303_SEE_OTHER
        )

    async def init_view(self, request: Request):
        all_user = await self.admin_model.all().limit(1)
        if len(all_user) > 0:
